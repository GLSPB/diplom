...
--
-- Хост: 127.0.0.1
-- Время создания: Ноя 15 2023 г., 10:27
...
CREATE TABLE `question_t` (
  ...
  `id_answer_question` int(11) NOT NULL,
...
CREATE TABLE `test_t` (
  ...
  `id_question_test` int(11) NOT NULL,
...
ALTER TABLE `question_t`
  ...
  ADD KEY `Question_t_fk0` (`id_answer_question`);
...
ALTER TABLE `test_t`
  ...
  ADD KEY `Test_t_fk0` (`id_question_test`),
...
--
ALTER TABLE `question_t`
  ADD CONSTRAINT `question_t_ibfk_1` FOREIGN KEY (`id_question`) REFERENCES `answer_t` (`id_question_answer`),
  ADD CONSTRAINT `question_t_ibfk_2` FOREIGN KEY (`id_answer_question`) REFERENCES `test_t` (`id_TEST`);
